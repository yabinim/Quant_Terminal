...
MARKERS = {
    ...
    "SATELLITE_MANDATE.md": ["β중립 C1", "β중립 결과", "대응 시도 1 — β중립 12-0",
                             "보강 시도 1 — 크레딧 게이트"],
    # ── C 트랙 PEAD 라벨 검증 (2026-09-11 · 약정 → 러너) ────────────────────
    # 약정 문서 · 러너 · Phase 0 프로브 2개가 한 묶음이다. 러너는 시작할 때
    # md 를 읽어 문턱·구간·seed 를 대조하므로, **둘 중 하나만 낡아도 러너가
    # 중단한다** — 시끄러운 실패다. 그래도 관문에서 먼저 잡는 편이 낫다:
    # 판정은 1회 전용이라, 낡은 사본으로 한 번 돌면 되돌릴 수 없다.
    # precommit_check/judge_tier/_pass_rate: 약정 대조 · 판정 게이트 · 오탐률
    #   재현. 셋 중 하나라도 없는 사본은 셀프테스트가 통과하지 못한다.
    "diag_pead_label_verify.py": ["precommit_check", "judge_tier", "_pass_rate"],
    # ⚠️ 아래 md 마커 3개는 전부 **약정 블록**에만 있다. 판정 결과를 §11 에
    #   기록한 뒤에는 그 행의 문구(예: "판정 실행 2026-")를 네 번째 마커로
    #   추가할 것 — 그래야 결과 기록 **전** 사본과 **후** 사본이 갈린다.
    #   SATELLITE_MANDATE.md 가 같은 이유로 셋째 마커를 갖고 있다.
    # 앞의 3개는 약정 블록에만 있어 결과 기록 **전/후** 사본을 가르지 못한다.
    #   네 번째가 그 구분이다(2026-09-11 20:32 판정 완주 · 두 라벨 미달).
    #   md 가 3/4 이면 결과 기록 전 사본이다 — 그 사본으로 §11 을 고치면 판정
    #   기록이 날아간다. SATELLITE_MANDATE.md 가 같은 이유로 셋째 마커를 갖는다.
    "PEAD_PRECOMMIT.md": ["강한 상승 반응", "무결성 중단", "결함 재실행",
                          "판정 실행 2026-09-11 20:32"],
    # 러너가 유니버스·표본·분기말을 이 둘에서 그대로 부른다(재구현 금지).
    ...
    # ⚠️ 아래 md 마커 4개 중 앞 3개는 **약정 블록**에 있고, "그리고 종료 판정" 만
    #   §9 종료 기록과 함께 들어왔다. md 가 3/4 이면 **종료 판정 기록 전** 사본이다
    #   — 그 사본으로 §9 를 고치면 검정력 표와 종료 근거가 통째로 날아간다.
    #   PEAD_PRECOMMIT.md 가 같은 이유로 네 번째 마커를 갖는다.
    # ln(2) — M&A 규모 제외 문턱. 미검증 — §7 이 허용한 사실 표시의 캡션 조건.
    #   635 — 3%p 를 80% 검정력으로 잡는 데 필요한 분기 수(≈159년), 종료의 근거 숫자.
    "ISSUANCE_PRECOMMIT.md": ["ln(2)", "미검증", "635", "그리고 종료 판정"],
