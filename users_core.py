...
────────────────────────────────────────────────────────────────
스키마 v4 (13열): ID, Password, Reason, Source, Status, Email, Alert_Radar,
...
- Alert_Earnings: 실적 레이더 메일(D-3 사전 경고 + 발표 후 결과) 수신 여부 ("Y"/"N")
...
_V3_DEFAULTS_GUEST = {k: _DEFAULTS_GUEST[k] for k in _NEW_V3_COLS}
_V3_DEFAULTS_ADMIN = {k: _DEFAULTS_ADMIN[k] for k in _NEW_V3_COLS}
...
def ensure_users_header_v4(ws) -> bool:
    ...
    기본값: 기존 알림(DRG)은 현행 유지를 위해 Y, 신규 기능은 동의 없이 켜지 않도록 N.
            관리자(yab)만 전부 Y.
    ...
    # 본문 행의 토글 열 채우기 (인덱스 0~7 기존 열은 손대지 않는다)
    body = vals[1:]
    if not body:
        return True
    new_cells = []
    for r in body:
        ...
        defaults = _DEFAULTS_ADMIN if uid == ADMIN_CONTENT_OWNER_ID else _DEFAULTS_GUEST
        row = (list(r) + [""] * NCOL)[:NCOL]
        filled = []
        for i, col in enumerate(_MIGRATED_COLS, start=_MIGRATE_START):
            cur = str(row[i] or "").strip()
            filled.append(cur if cur else defaults[col])
        new_cells.append(filled)
    start_col = chr(ord("A") + _MIGRATE_START)      # "I"
...
# 하위 호환 별칭 — 기존 호출부(app.py / automation)가 깨지지 않도록 유지
def ensure_users_header_v3(ws) -> bool:
    """(레거시 별칭) v4 마이그레이션을 수행한다."""
    return ensure_users_header_v4(ws)


def ensure_users_header_v2(ws) -> None:
    """(레거시 별칭) v4 마이그레이션을 수행한다."""
    ensure_users_header_v4(ws)
