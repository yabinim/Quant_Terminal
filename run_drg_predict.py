...
def fetch_macro_context(fred: Fred) -> str:
    ...
    # ── 신호 4: 프리마켓 방향 (SPY/QQQ stock-price-change API) ─────────────
    # FMP는 ES=F/NQ=F 미지원 → stock-price-change + quote 폴백으로 SPY/QQQ 사용
    try:
        _fut_chgs = []
        _fut_lines = []
        # 1차: stock-price-change (1D)
        try:
            r_chg = requests.get(
                f"{FMP_BASE}/stock-price-change?symbol=SPY,QQQ&apikey={FMP_KEY}", timeout=8
            )
            if r_chg.status_code == 200:
                for item in r_chg.json():
                    sym = str(item.get("symbol", "")).upper()
                    chg1d = item.get("1D") or item.get("1d") or item.get("day")
                    if sym in ("SPY", "QQQ") and chg1d is not None:
                        chg = float(chg1d)
                        _fut_chgs.append(chg)
                        label = "S&P500(SPY)" if sym == "SPY" else "나스닥(QQQ)"
                        _fut_lines.append(f"{label} {chg:+.2f}%")
        except Exception:
            pass
        # 2차: quote 폴백
        if not _fut_chgs:
            for sym in ["SPY", "QQQ"]:
                try:
                    r_q = requests.get(
                        f"{FMP_BASE}/quote?symbol={sym}&apikey={FMP_KEY}", timeout=8
                    )
                    if r_q.status_code == 200:
                        d_q = r_q.json()
                        item = d_q[0] if isinstance(d_q, list) and d_q else d_q
                        chg_pct = item.get("changesPercentage") or item.get("changePercent")
                        if chg_pct is None:
                            price = float(item.get("price") or 0)
                            prev  = float(item.get("previousClose") or 0)
                            chg_pct = (price / prev - 1) * 100 if prev > 0 and price > 0 else None
        ...
        if _fut_chgs:
            ...
            lines.append(f"- 프리마켓 방향: {' / '.join(_fut_lines)} (평균 {avg_chg:+.2f}%) [{status}]")
