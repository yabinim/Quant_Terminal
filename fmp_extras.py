...
#   상한(15년 ≈ 3,764봉).
#
#   ⚠️ 2026-09-10 이전 값은 1826(≈5년)이었고 주석은 "limit 이 무시되던 시절의
#      사실상 동작과 같은 크기"라고만 말했다. 그 값이 **API 한계로 읽히면서**
#      저장소 여섯 곳 이상이 "FMP 자체 이력 상한 ~1,254봉"을 사실로 적게 됐다.
#      그 "사실상 동작"은 `from` 없이 보낸 요청에 FMP 가 주는 **기본 응답 창
#      (≈5년)** 이다. 기본값을 상한으로 읽은 것이 착오의 뿌리다 — 저장소의
#      "기본 창" 표기는 전부 이 뜻이다.
#      실측(automation/diag_hist_ceiling.py, 2026-09-10): 단일 호출의 실제 상한은
#      **롤링 5,000 레코드(≈19.8년 ≈ 7,277달력일)** 다. AAPL/MSFT/SPY/QQQ 가 전부
#      5,000봉·같은 최소일을 냈고 두 실행 사이 하루가 지나자 바닥도 하루 밀렸다.
#      `to` 를 과거로 미니 창이 따라 이동했고, 19개 완전 연도의 봉수는 250~253
#      이었다(중간 절삭 없음).
#      즉 1826 은 우리가 스스로 건 정책 상수였다. 그리고 hist_days_for_bars() 가
#      거기서 클램프하는 바람에 **저장소 어디에서도 그보다 깊은 요청이 나가지
#      않아 자기 반증이 구조적으로 막혀 있었다.**
#
#   왜 7,277 이 아니라 5,478 인가 — 정책 상수는 "쓸 수 있는 최대"가 아니라
#      "필요를 덮되 API 상한에 안 닿는 값"이어야 한다. 상한에 정확히 닿아 있으면
#      FMP 가 5,000 → 4,500 으로 조용히 낮췄을 때 알아챌 방법이 없다.
#      5,478 은 3,764봉으로 실측 상한 대비 **1,236봉(25%) 여유**가 있고,
#      hist_days_for_holding 이 **보유 13년까지 절삭 없이** 덮는다(옛 값에서는
#      보유 4년부터 잘렸다 — 트레일링 고점이 낮아져 매도 신호가 덜 떴다).
#
#   ⚠️ 이 값을 올릴 때 확인할 것: 상한에 걸려 있던 호출부의 행동이 **조용히**
#      바뀐다. 2026-09-10 시점에 걸려 있던 곳은 둘뿐이었다 —
#      diag_satellite_backtest(HISTORY_BARS=1300)와 4년 이상 보유 종목.
#      전자는 §4① 판정 재현을 위해 같은 파일의 WINDOW_DAYS_PIN 으로 1826 을
#      유지한다. 이 상수를 다시 올릴 때도 같은 점검을 먼저 할 것.
#      ※ 정정: "둘뿐"은 틀렸다 — 초과폭이 작아 놓친 곳이 셋 더 있었다.
#        run_signal_backtest(1255봉 → 1,834일, 8일 초과: 월간 백테스트가 ~6봉 더
#        받는다. 평가 구간 = 마지막 TEST_LOOKBACK 봉이라 그대로, 앞쪽 워밍업만
#        길어졌다), diag_earnings_preview_backtest(1250봉 → 1,827일, 1일 초과),
#        diag_fmp_depth(run_signal_backtest 설정 차용). 점검은 bars 가 아니라
#        hist_days_for_bars(bars) > 옛 상한 으로 할 것 — pad 5봉이 경계를 넘긴다.
#
#   재검증: automation/diag_hist_ceiling.py (수동 워크플로). 플랜이 바뀌면
#      실측 상한도 바뀔 수 있다 — 이 숫자는 관측이지 계약이 아니다.
...
def _closes(ticker: str, bars: int) -> pd.Series:
    ...
    [2026-08-28] `limit`(170·260) → `from`/`to` 창. FMP 는 historical-price-eod 의
      `limit` 을 **무시**하므로 두 호출부 모두 1,254봉(기본 창)을 받고 있었다.
...
def mom_vol_annualized(vals, bars: int = MOM_VOL_BARS) -> float:
    ...
    MSCI 원본과 다른 점 두 가지 — 재현이 아니라 **근사**다:
      · MSCI 는 **3년 주간** 수익률을 쓴다. 여기는 **1년 일간**이다.
        ...
        1,255 − 756 ≈ 499봉(≈2년)만 남는다(§4① 핀 창 1,826달력일 기준 —
        FMP 상한이 아니다). 252봉 창 6개를 그 안에 넣으면
