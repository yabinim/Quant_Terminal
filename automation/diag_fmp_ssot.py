...
_RAW_GET_BASELINE = {
    ...
    # 2026-09-04b: 50 → 47. **grades-consensus 3곳을 fetch_grades_consensus
    #   하나로 합쳤다.** 같은 엔드포인트·같은 티커(selected_ticker)·같은 5필드를
    #   파싱하는 코드가 세 벌 있었다 — 변수명만 `_sb2/_b2/…` · `_rsb/_rb/…` ·
    #   `sb/b/…` 로 달랐다.
    #   ⚠️ 여기서도 이유는 스타일이 아니다. 셋 중 둘은 **모듈 최상위**였고
    #   나머지 하나(calculate_style_scores)도 @st.cache_data 없는 함수라
    #   모듈 최상위에서 불렸다. Streamlit 은 위젯을 건드릴 때마다 스크립트를
    #   전부 다시 돌린다 — **사이드바 입력 한 번에 FMP 콜이 하나씩 나갔고**
    #   레이트리미터 밖이었다. 새 헬퍼는 @st.cache_data(ttl=1800) + fh 경유다.
    #   ⚠️ 같은 패스에서 `_fmp_macro_get` → `_fmp_ui_get` 으로 개명했다.
    #   재시도 1회의 근거가 "매크로"가 아니라 "대화형 경로"였기 때문이다.
