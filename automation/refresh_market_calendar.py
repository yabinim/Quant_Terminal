...
def add_reminder(sh, title, what, why, source):
    """캘린더 이상 발견 시 Reminders 에 항목 추가.

    두 종류가 여기로 온다 — **임시 휴장**(규칙이 구조적으로 못 잡는 것)과
    **반일장 규칙 이탈**(규칙이 있는데 FMP 와 어긋난 것). 성격은 다르지만
    배달 경로가 같아야 한다: 둘 다 로그에만 남기면 아무도 안 본다.
...
def main():
    ...
    # ── STEP 5: 반일장 규칙 이탈이면 리마인더 ────────────────────────────
    # 왜 필요한가 — 배달 경로의 비대칭
    # ────────────────────────────────
    # 임시 휴장(extra)은 바로 위에서 Reminders 시트로 배달되는데, 반일장
    # 불일치는 STEP 2 에서 print 만 했다. 이 잡은 주 1회 GitHub Actions 로
    # 도는 배치라 **사람이 로그를 열지 않으면 아무도 모른다.** 규칙이 FMP 와
    # 어긋나도 Actions 는 초록불로 끝난다 — 조용한 실패다.
    #
    # 이 규칙이 틀리면 무엇이 조용히 깨지나:
    #   · app.py 시장 상태 헤더가 반일장을 "🟢 Regular Market" 으로 표시
    #   · 2PM 워치리스트 가드가 안 걸려 13:00 **종가**가 "장중 잠정" 으로 발송
    #   · 세션 라벨이 마감 후를 "Market Hours Analysis" 로 시트에 저장
    # 셋 다 숫자는 맞고 라벨만 틀린다 — 예외도 에러 로그도 남지 않는다.
    # 그래서 발견 즉시 배달돼야 한다.
    #
    # 중복 생성은 add_reminder 가 제목 기준으로 막는다. 제목에 날짜가 들어가
    # 있으므로 같은 불일치가 매주 재발해도 항목은 하나만 열린다.
    if _fo or _ro or _td:
        _hd = ", ".join(sorted(set(_fo) | set(_ro) | set(_td)))
        add_reminder(
            sh,
            title="반일장 규칙 불일치 — " + _hd,
            what=("FMP 의 조기 마감 시각과 calendar_core.nyse_early_close_days "
                  "규칙 계산이 어긋납니다. NYSE 공식 일정으로 어느 쪽이 맞는지 "
                  "확인하고, FMP 가 맞으면 규칙을 고치세요. 고친 뒤 "
                  "diag_market_calendar.py 를 돌려 골든 7년이 여전히 통과하는지 "
                  "확인해야 합니다. 해당 날짜: " + _hd),
            why=("반일장 판정은 규칙 계산이 **단독으로** 합니다(핫 패스라 시트를 "
                 "못 읽습니다). app.py 시장 상태 헤더 · 2PM 장중 가드 · 세션 "
                 "라벨 셋이 전부 이 규칙 하나에 걸려 있고, 틀려도 로그에 에러가 "
                 "남지 않습니다. 이 주간 대조가 유일한 발견 경로입니다."),
            source="refresh_market_calendar.py",
        )
