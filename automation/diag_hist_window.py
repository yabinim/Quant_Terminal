...
def group_H():
    ...
    cases = [
        ...
        # ⚠️ 2026-09-10 이전에는 여기가 `days_ago(2200), MX` 였다 —
        #    "6년 보유는 상한에 잘린다"가 참이던 시절(MX=1826)의 값이다.
        #    상한이 5478 로 오르면서 그 전제가 깨졌다. 숫자를 하드코딩하면
        #    상한이 바뀔 때마다 이 표가 옛 세계를 강제하게 된다 — H7 처럼
        #    MX 로 파라미터화한다.
        ("H6", days_ago(MX), MX, "상한 초과 보유 → 상한"),
        # H6b 는 2026-09-10 절삭 버그 수정의 **회귀 앵커**다. 옛 상한에서는
        # 보유 4년부터 창이 잘려 `compute_position_drawdown` 의 보유 고점이
        # 실제보다 낮아졌고, 그만큼 트레일링 스톱 매도 신호가 덜 떴다.
        # 이 줄이 빨간불이면 그 버그가 되살아난 것이다.
        ("H6b", days_ago(2200), B + 2200, "6년 보유 → 절삭 없음(버그 수정 앵커)"),
