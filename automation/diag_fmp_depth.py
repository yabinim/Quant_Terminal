...
가설: FMP 가 limit 을 올려도 그만큼의 봉을 주지 않는다(계정 플랜의 이력 한도).
  → 2026-09-10 기각: 5년은 `from` 없는 요청의 기본 창이었다. 실제 상한은 롤링
    5,000 레코드(diag_hist_ceiling). 'limit 무시'는 여전히 참이다(§7).
...
(§7 이 박은 것은 'limit 무시'다. ~1,255봉은 `from` 없는 요청의 기본 창이지
이력 상한이 아니다 — 거기서 '상한 ~1,255봉'을 유도한 것이 틀렸다.)
...
살아 있는 질문이다 — 신규 상장·이력 짧은 종목은 창 깊이와 무관하게 얕다.
...
def main() -> int:
    ...
    # v2.9: 예전엔 max(HISTORY_BARS, 3000) 으로 '넉넉히' 요청했다. 당시엔
    #       hist_days_for_bars 가 HIST_MAX_DAYS(1826일)에서 클램프해 1255봉이든
    #       3000봉이든 같은 창이 나갔다. 2026-09-10 부터 상한이 5478일이라 3000봉이면
    #       창이 실제로 넓어진다 — 그래도 재야 할 것은 백테스트가 쓰는 설정값이다.
    #       실제 설정값으로 잰다.
