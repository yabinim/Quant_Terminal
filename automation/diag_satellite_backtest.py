...
────────────────────────────────────────────────────────────
 ...
 2) **구간 편향**: 조회 창이 1,826달력일(≈1,255봉 · WINDOW_DAYS_PIN)이라 2020 코로나·
    2022 초입 하락장이 평가 구간에 없다. 커버 구간은 대체로 강세장이다.
    (예전엔 "FMP 계정 이력 한도"로 적었으나 한도가 아니었다 — 핀을 풀면 들어온다.)
...
#   ⚠️ 이 값을 올려도 받는 봉수는 안 늘어난다. WINDOW_DAYS_PIN(1826일 ≈ 1,254봉)이
#      창을 자른다(2026-09-10 전에는 fmp_extras.HIST_MAX_DAYS 가 같은 일을 했다).
#      예전 주석의 "FMP 자체 상한(~1,255봉)"은 없다 — FMP 상한은 롤링 5,000 레코드.
...
#        받는 봉수 ≈ 1,254 (WINDOW_DAYS_PIN 1826일)
...
#      핀을 풀면 4 는 데이터를 버리게 된다. 캡은 캡으로 둔다.
...
def _window_days_for(bars: int, warn=print) -> int:
    ...
    (2026-09-10 부터 그 자리는 WINDOW_DAYS_PIN 이 잇는다 — 경고도 핀을 밝힌다.)
...
def _fmp_eod(ticker: str, endpoint: str, *, bars: int, as_of) -> tuple:
    ...
    v2.9: `&limit=` 을 `from`/`to` 창으로 바꿨다.
      ...
      항상 돌려주는 기본 창(~1,255봉)보다 커서 늘 그 창을 통째로 받았을 뿐이다.
...
def main() -> int:
    ...
    print("   2) 조회 창 1,826달력일(WINDOW_DAYS_PIN)이라 2020 코로나·2022 초입 하락장이 "
          "평가 구간에 없다.")
