#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""diag_credit_gate_probe.py — 크레딧 스프레드 게이트 프로브 (읽기 전용 · **종료**)

═══════════════════════════════════════════════════════════════════════════
종료 판정 — 2026-09-10. 생존 후보 0. 이 신호는 닫혔다.
═══════════════════════════════════════════════════════════════════════════
사전 약정(P5 off비율 · P6 에피소드 · P1 겹침)은 v2 실행 **전에** 이 파일에
박혀 있었고, 네 후보 × 두 시리즈가 전부 떨어졌다.

  종료 근거는 사전 약정보다 **전제 붕괴** 쪽이 강하다.
  이 프로젝트의 전제는 "주식보다 신용이 먼저 조인다" 였다. BAA10Y 19.7년,
  짝 30~60개에서 리드/래그 중앙값이 —
      C1 ma20 +0일(짝 60) · C2 pct80 -2일(짝 31) · C4 z10 -1일(짝 35)
      C3 chg60 -8일(짝 3)
  **전부 0 또는 음수다.** 신용은 먼저 조이지 않는다. v1 의 +22일은 짝 3개짜리
  잡음이었다(그때 '짝 3'을 같이 찍어둔 것이 값을 했다).

  ⚠️ 리드/래그는 출력 전용 열이다. 여기서 쓰는 방향은 **실패에 확신을 더하는**
     쪽이지 실패를 되살리는 쪽이 아니다 — mom12_1_ra 의 "확신을 낮출 수만
     있고 높일 수는 없다"와 같은 비대칭이다.

  P5 를 풀어도 결론은 안 바뀐다. C2·C4 가 살아나도 J 0.35~0.49 ·
  P(off|spy_off) 0.52~0.67 이다. SPY200 이 꺼진 날의 절반~2/3 에서 같이
  꺼지면서 타이밍 이득이 0 인 지표는 두 번째 축이 아니라 200일선의 흐릿한
  복사본이다.

  ⚠️ P5 의 5~15% 앵커 자체가 틀렸다는 것도 기록해 둔다. 근거로 쓴 "SPY200
     10.0%" 는 v1 의 2.2년 창 값이고, 19.7년으로 재면 **23.5%** 다. 그럼에도
     P5 를 다시 잡지 않았다 — 직전 라운드(R1 의 36%)에서 이미 "이건 재협상이
     아니다"를 한 번 썼고, 두 번 연속 쓰면 사전 약정은 아무것도 구속하지
     않았던 것이 된다. 명분과 실익이 둘 다 없다.

  깨끗한 점: HY OAS 에서 '판정불가'였던 C4 가 BAA10Y 19.7년에서는 24.5% 로
  **탈락** 판정을 받았다. "데이터만 더 있었으면"이 남지 않는다.

재개하려면 **먼저 새 사전 약정을 걸어야 한다.** 데이터가 늘었다는 사실만으로
다시 여는 것은 이 판정을 무의미하게 만든다.

프로덕션 코드 변경은 **0** 이다 — 이 프로브는 처음부터 읽기 전용이었다
(산업 모멘텀 종료·실적 프리뷰 종료와 같은 모양).

이 실행이 남긴 **별건 소득 둘** — 크레딧 게이트보다 값이 나간다:
  ① `fx.HIST_MAX_DAYS = 1826` 은 API 한계가 아니라 정책 상수다. 단일 호출로
     **4,952봉 · 19.7년**(2007-01-03~)이 왔다. 저장소 여러 곳이 "FMP 자체
     상한 ~1,255봉"을 사실로 적고 있는데 그게 반증됐다. 별도 항목으로 처리 중.
  ② `BAMLH0A0HYM2` 는 API 에서 **3년으로 잘린다.** observation_start 를
     1996-01-01 로 명시해도 786점 / 2023-09-11 그대로다 — 호출 방법 문제가
     아니라 시리즈 쪽 제한이다. 전장 대체는 `BAA10Y`(40.7년, 지연 2일).
     다시 시도하지 말 것.
...
═══════════════════════════════════════════════════════════════════════════
v2 실측 기록 — 2026-09-10 06:5x UTC. 지우지 않는다.
═══════════════════════════════════════════════════════════════════════════
  Q1  창없음 1253봉 · 1826일 1254봉 · 2007~2009 756봉
      · **D3 단일 장기 호출 4952봉 / 2007-01-03~2026-09-09 (19.7년)**
  Q2  BAMLH0A0HYM2 786점 2023-09-11~ (start 명시해도 동일) → 불가
      BAA10Y 10171점 1986-01-02~ (40.7년, 지연 2일) → 충분 · NFCI 2904점
  Q3  [BAMLH0A0HYM2 / 공통 751봉]
        C1 551봉 36.1% 16회 J0.187 탈락 · C2 247봉 3.6% 2회 J0.375 탈락
        C3 551봉 3.6% 1회 J0.364 탈락 · C4 499봉 6.0% 1회 J0.518 판정불가
        SPY200: 55일 10.0% · 3회
      [BAA10Y / 공통 4913봉 · 2007-01-03~]
        C1 4713봉 47.6% 130회 J0.276 리드+0(짝60) 탈락
        C2 4409봉 18.8%  33회 J0.354 리드-2(짝31) 탈락
        C3 4713봉  2.4%   2회 J0.104 리드-8(짝 3) 탈락
        C4 4661봉 24.5%  30회 J0.486 리드-1(짝35) 탈락
        SPY200: 1109일 **23.5%** · 60회   ← P5 앵커가 틀렸다는 증거
  판정 생존 0 → 종료.
