...
배경
────
`limit` 은 무시된다(limit=500 → 기본 창 1254봉). v1 프로브로 **`from`/`to` 는 먹힌다**가
...
─────────────────────────────────────────────────────
현재 호출부의 `limit` 값들은 **전부 무의미했다.** 뭘 적든 1254봉(기본 창)이 왔다.
...
[D] 완전성 — 좁은 창만으로는 못 본다
  ...
  D2 전폭 창  : from = 기준선의 최초 날짜 (기준선 전량 ≈ 1254봉 · API 상한 아님)
