...
#         요청과 무관하게 항상 ~1,254봉(기본 창)이 왔다. 러너·코어는 이미 from/to
#         창으로 옮겼는데 이 진단만 남아 있었다.
#      ② 1400 은 실제로 받던 봉수(~1,254)보다 컸다 — 요청값과 수신값이 달랐는데
#         아무도 몰랐다. 검증된 요구치가 아니라 limit 이 작동하는 줄
#         알고 적은 숫자다.
#         ※ 2026-09-10 정정: 여기엔 "1400 은 FMP 상한(~1,254봉)을 넘는 값"이라고
#           적혀 있었다. 틀렸다 — FMP 상한은 롤링 5,000 레코드다.
...
# 종목당 이벤트는 분기 실적이라 63봉 간격. 창 1,250봉이면 약 18건이다.
# 즉 **창 깊이가 곧 표본 수**다 — 창을 늘리면 이벤트가 더 나온다.
#   (예전 주석은 1,254봉을 "상한"으로 적었다. 옛 HIST_MAX_DAYS=1826 클램프 값이었을
#   뿐 FMP 상한이 아니다 — FMP 상한은 롤링 5,000 레코드. 늘릴지는 별도 판단이다.)
HIST_BARS = 1250          # 옛 상한(1826일) 부근. 2026-09-10 부터 클램프 없이 요청대로 받는다.
