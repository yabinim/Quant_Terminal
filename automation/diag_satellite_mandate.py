...
# K12 — 결과(2026-09-11 · 미관찰 → 종료)를 기록한 §2 각주가 종료를 잃지 않았는가.
#       J8 과 같은 이유다. 2009 반등 Δ +29.0%p 는 눈에 띄는 숫자라, 나중에 종료 범위
#       문장만 지우고 "잔차 모멘텀이면 다른 거니까"로 붕괴 완화를 다시 재기 쉽다.
#       ⚠️ 이 각주에는 `**β중립 C1**` 같은 굵은 라벨을 쓰지 않는다. K 는 문서의 **첫**
#          매치를 읽고 §2 가 §4① 보다 앞이라, 라벨을 쓰면 K5·K10 이 약정 대신 여기를 읽는다.
#       위치도 본다 — §2(깊은 창 약점 각주 뒤 · §3 앞) 밖으로 옮기면 약점 옆에서 사라진다.
_K12_HEAD = "**대응 시도 1 — β중립 12-0"
_K12_KEEP = ("미관찰 → 종료", "그대로 남는다", "이 종료의 재시험으로 본다", "재시험 금지")


def md_beta_note_ok(text):
    t = text or ""
    i = t.find(_K12_HEAD)
    h = t.find(_J8_HEAD)
    j = t.find("### 3.", i if i >= 0 else 0)
    return 0 <= h < i < j and all(k in t[i:j] for k in _K12_KEEP)


chk("K12 §2 각주가 β중립 종료 기록·종료 범위·'재시험 금지'를 §3 앞에서 유지한다",
...
# ── K12 양성 대조 ───────────────────────────────────────────────────────
# ⚠️ 치환 앵커가 §2 각주에만 있어야 한다. "재시험 금지" 는 §4① 결과 줄에도 있어
#    앵커로 쓰면 약정 쪽이 먼저 지워지고 이 대조는 엉뚱한 곳을 시험한다.
chk("H18 종료 범위 문장만 지운 md 는 K12 를 통과하지 못한다",
    _would_fail(lambda: md_beta_note_ok(MD.replace("이 종료의 재시험으로 본다", "", 1))), True)


def _move_beta_note(text, to):
    """각주를 떼어 `to` 헤더 바로 앞에 붙인다 — 내용은 그대로, 위치만 바뀐다."""
    i = text.find(_K12_HEAD)
    j = text.find("### 3.", i)
    if i < 0 or j < 0:
        return text
    note = text[i:j]
...
# 두 방향 모두 본다 — §3 뒤(§4 앞)로 내리는 것과, 깊은 창 약점 각주보다 위(§2 머리)로
# 올리는 것. 한쪽만 보면 위치 조건의 절반이 뮤테이션 검증 없이 남는다.
