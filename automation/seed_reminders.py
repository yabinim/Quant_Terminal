"""seed_reminders.py — 개발 리마인더 7건 적재 (멱등 · 재실행 가능).
...
SEEDS = [
    ...
    rmc.make(
        title="위성 실행 기록 배선 확인",
        due="2026-09-21",
        category="검증",
        what_to_check=(
            "⚠️ **판정하지 않는다. 배선만 본다.** "
            "① Satellite_Instruction 시트에 대상일 2026-09-11 의 2행(A반·B반)이 "
            "있는가. 없으면 9/13 run_hidden_alpha 로그에서 '[WARN] 지시 기록' 을 찾는다. "
            "② Trade_History 에 hsa-위성-12-0 · hsa-위성-blend 계좌의 9/14 이후 "
            "체결이 실제로 남았는가. **집행했는데 행이 없다면 그것이 이 항목의 핵심이다.** "
            "③ 남지 않았다면 거래를 기록하는 절차부터 세운다 — 층 2 의 '실행' 쪽 "
            "입력이 통째로 사라진다. 지시만 쌓이고 실행이 비면 대조할 것이 없다. "
            "④ Risk_On 이 공란이면 그 주 SPY 200봉을 못 받은 것이다. 두 주 연속 "
            "공란이면 원인을 본다. "
            "⑤ Pool_N 이 10 안팎인가 — 급감했다면 섹터 챔피언 선별이 깨진 것이다."
        ),
        why=("지시 기록(층 1)은 2026-09-08 에 코드로 만들었지만, 실행 쪽은 "
             "Trade_History 에 사용자가 직접 넣는 데 의존한다. 코드가 아니라 "
             "습관이라 조용히 비어 있을 수 있고, 비어 있으면 층 2 를 아무리 잘 "
             "만들어도 잴 것이 없다. 11-03 에 발견하면 8주치가 이미 날아간 뒤다."),
        source="SATELLITE_MANDATE §4② 층 1 구축(2026-09-08)",
    ),
    rmc.make(
        ...
        what_to_check=(
            ...
            "② 9/13 이후 **주간 리밸런싱** 지시를 실제로 실행했는가. "
            "Satellite_Instruction 시트의 Top_JSON 과 Trade_History 의 체결을 "
            "주 단위로 대조한다 — 건너뛴 주가 있는가. "
            ...
            "⑥ **층 2(실행 대비 이탈 판정) 착수 판단.** 이 시점이면 "
            "Satellite_Instruction 에 8주치가 쌓였을 것이다. 그 기록을 놓고 "
            "두 숫자를 **먼저 확정한 뒤** 구현한다 — (a) 유예 기간: 지시 대상일로부터 "
            "며칠 안에 집행하면 준수로 볼 것인가. (b) 부분 이탈: 5칸 중 일부만 "
            "채운 주, Risk_On=False 라 일부러 안 산 주를 각각 어떻게 셀 것인가. "
            "이 둘은 사전 확약이며 나중에 결과를 보고 바꾸지 않는다 (§5 와 같은 규율). "
            "실행 기록이 비어 있으면 착수하지 말고 배선부터 고친다 — 09-21 항목."
        ),
        why=("원래는 실거래 3개월로 로테이션을 재평가하려던 항목이다. "
             "2026-09-07 A/B 분할로 측정 대상이 바뀌었고, 지시 기록(층 1)은 "
             "2026-09-08 에 생겼지만 판정(층 2)이 없어 전략 문제와 실행 문제를 "
             "아직 수치로 가를 수 없다. "
             "그래서 성격을 성과 판정에서 실행 점검으로 바꿨다. "
             "당시 리밸런싱 1주 누락과 USO 를 로테이션 신호 이후까지 보유한 건이 있었다."),
        source="2026-08-03 결정 → 2026-09-07 A/B 분할 → 2026-09-08 §1 주간 정정·층 1 구축",
    ),
    rmc.make(
    ...
    rmc.make(
        ...
        what_to_check=(
            ...
            "③ 실행 이탈 기록이 켜져 있으면 지시대로 굴린 주만 골라서 본다. "
