...
_FMP_TIMEOUT = 20            # v2.8: 7(fmp_http 기본)·8 은 5년치(≈1,260봉) 페이로드에 짧다.
...
TEST_LOOKBACK  = 934           # v2.5: 평가 구간(거래일 ≈ 3.7년) — 당시 받던 1,255봉에서 역산한 값.
#   진단(diag_fmp_depth) 결과: FMP 는 limit 값과 무관하게 항상 1255봉(정확히 5년,
#   롤링)을 반환했다. 500 을 요청하든 5000 을 요청하든 시작일이 동일했다.
#   ※ 그 5년은 `from` 없는 요청의 기본 창이었다. 계정 한도가 아니다 — 실제 상한은
#     롤링 5,000 레코드(diag_hist_ceiling 2026-09-10, → fmp_extras.HIST_MAX_DAYS).
...
#   → 2018 Q4·2020 코로나 폭락이 창 밖이라 아웃오브샘플 검증이 불가능했다
#     (당시엔 "데이터 자체가 없다"고 적었으나 틀렸다 — 창을 넓히면 들어온다).
#   당시 가능한 최대 = 1255 − MIN_PRIOR_BARS(220) − 꼬리(101) = 934.
...
#   해결. 934 는 당시 받던 1,255봉에서 나온 값이다. 지금은 창이 HIST_MAX_DAYS
#   (5478일 ≈ 3,764봉)까지 열려 있어 934 는 한도가 아니라 **선택**이다 — 올리면
#   HISTORY_BARS 가 따라 올라가고 과거 판정(전·후반 분할 등)과의 대조가 끊긴다.
...
#      1255봉 요구 + pad 5봉 → 1,834달력일 ≈ 1,260봉이 온다. 1154 대비 여유
#      ≈106봉이므로 평가 구간 934일은 온전하다.
#      (2026-09-10 전에는 HIST_MAX_DAYS=1826 클램프에 pad 가 먹혀 ≈1254봉이었고,
#      이 자리에 "TEST_LOOKBACK 이 1034 를 넘으면 FMP 5년 한도가 먼저 막는다"고
#      적혀 있었다. 그런 한도는 없다 — TEST_LOOKBACK 을 올리면 HISTORY_BARS 가
#      따라 올라가고, 창을 막는 것은 HIST_MAX_DAYS(5478일 ≈ 3,764봉)다.)
...
# v2.5 전·후반 분할: 평가 구간(TEST_LOOKBACK)에 2018/2020 이 없어 진짜 아웃오브샘플은
#   불가능하다(당시 "FMP 5년 한도" 탓으로 적었으나 한도가 아니었다 — TEST_LOOKBACK 주석).
...
def _print_rt_summary(rt_aggs: dict, meta: dict) -> None:
    ...
    if any(k[2] == "half1" for k in rt_aggs):
        ...
        print("   평가 구간(TEST_LOOKBACK)에 2018·2020 하락장이 들어 있지 않다.")
